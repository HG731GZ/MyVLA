...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Freeze only the VLM backbone for this many optimizer steps",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Attention/MLP dropout used by the Cross-Self action head",
    )
    parser.add_argument(
        "--num_views",
        type=int,
        default=2,
        help="Number of fixed image slots; use 2 for the Cross-Self Only experiment",
    )
    parser.add_argument(
        "--language_max_length",
        type=int,
        default=96,
        help="Maximum number of instruction tokens",
    )
...
def build_optimizer(
    ...
    """Build explicit, exhaustive VLM and action-head parameter groups."""
    del lr_coef_vlm
    vlm_params = []
    action_head_params = []
    unassigned = []
    ...
    if unassigned:
        raise ValueError(f"Optimizer has unassigned parameters: {unassigned}")
    assigned_ids = [id(p) for p in vlm_params + action_head_params]
...
def update_group_lrs(optim, step, args):
    ...
    vlm_base_lr = args.learning_rate * args.learning_coef
    action_head_base_lr = args.learning_rate

    if step < args.freeze_vlm_steps:
        set_group_lr(optim, "vlm", 0.0)
    else:
        vlm_lr = (
            linear_warmup_cosine(
                step,
                args.freeze_vlm_steps,
                args.warmup_steps,
                args.iters,
                vlm_base_lr,
                args.min_lr_ratio,
    ...
    action_head_lr = (
        ...
        if args.use_cosine_decay
        else action_head_base_lr
    )
    set_group_lr(optim, "action_head", action_head_lr)
...
def main(args):
    ...
    if args.num_views < 2:
        raise ValueError("--num_views must be at least 2 for AgentView and WristView")
    expected = (768, 9, 12)
    actual = (args.hidden_size, args.depth, args.num_heads)
    if actual != expected:
        raise ValueError(
            "The Cross-Self action head requires "
            f"hidden_size/depth/num_heads={expected}, got {actual}."
        )
    ...
    tracker_config = {
        ...
        "freeze_vlm_steps": args.freeze_vlm_steps,
    ...
    if load_path and os.path.isdir(load_path) and os.path.exists(os.path.join(load_path, "model.safetensors")):
        ...
        if args.action_mode != model.action_mode:
            raise ValueError(
                f"Checkpoint action_mode={model.action_mode!r} does not match "
                f"--action_mode={args.action_mode!r}."
            )
        ...
        if args.num_actions != model.num_actions:
            raise ValueError(
                f"Checkpoint num_actions={model.num_actions} does not match "
    ...
    action_head_parameters = count_parameters(model.transformer)
    vlm_parameters = count_parameters(model.vlm)
    logger.info(f"VLM parameters: {vlm_parameters:,}")
    logger.info(f"Cross-Self action-head parameters: {action_head_parameters:,}")
    ...
    for batch in train_dataloader:
        with accelerator.accumulate(model):
            ...
            freeze_vlm = global_step < args.freeze_vlm_steps
            set_group_trainable(
                optim,
                "vlm",
                (not freeze_vlm) and args.learning_coef != 0.0,
            )
            set_group_trainable(optim, "action_head", True)
        ...
        if global_step % args.log_interval == 0:
            ...
            if accelerator.is_main_process:
                ...
                logger.info(
                    ...
                    f"lr_action={logs['lr_action_head']:.2e} "

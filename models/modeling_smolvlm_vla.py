...
from .transformer_smolvlm import SmolVLMCrossSelfActionTransformer
...
def _pack_vlm_inputs(
    full_image_features: torch.Tensor,
    image_mask: torch.Tensor,
    text_embeds: torch.Tensor,
    text_attention_mask: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Pack valid image patches and text tokens into a padded batch."""
    B, _, _, hidden_size = full_image_features.shape
    packed = []
    max_seq_len = 0

    for b in range(B):
        valid_indices = image_mask[b].nonzero(as_tuple=False).flatten()
        sample_image_feats = full_image_features[b, valid_indices].reshape(
            -1,
            hidden_size,
        )
        sample_text_embeds = text_embeds[b, text_attention_mask[b]]
    ...
    padded = full_image_features.new_zeros(B, max_seq_len, hidden_size)
    ...
    for b, embeds in enumerate(packed):
        seq_len = embeds.shape[0]
        padded[b, :seq_len] = embeds
...
class SmolVLMVLA(PreTrainedModel):
    ...
    Components:
      ...
      • Cross-attention/self-attention flow-matching action head
    ...
    def __init__(self, config: SmolVLMVLAConfig, *args, **kwargs):
        ...
        self.transformer = SmolVLMCrossSelfActionTransformer(
            ...
            max_num_actions=config.num_actions,
            dropout=config.attention_dropout,
        )
        logging.info("Cross-Self action head enabled")
    ...
    def forward_vlm_efficient(
        ...
        if image_mask.shape != (B, V):
            raise ValueError(
                f"image_mask must have shape {(B, V)}, got {tuple(image_mask.shape)}."
            )
        if not torch.all(image_mask.any(dim=1)):
            raise ValueError("Each sample must contain at least one valid image view.")
        if input_ids is None:
            raise ValueError("input_ids are required for forward_vlm_efficient")
        if text_attention_mask is None:
            ...
            if text_attention_mask.shape != input_ids.shape:
                raise ValueError(
                    "text_attention_mask must match input_ids shape, got "
                    f"{tuple(text_attention_mask.shape)} and {tuple(input_ids.shape)}."
                )
        ...
        padded_inputs_embeds, attention_mask = _pack_vlm_inputs(
            full_image_features,
            image_mask,
            text_embeds,
            text_attention_mask,
    ...
    def forward(
        ...
        v_t = self.transformer(
            ...
            vlm_attention_mask=enc["vlm_attention_mask"],
        )
